...
export class FileSystem {
  ...
  _setBaseFilePath(fileDirName = null) {
    let baseFilePath = (this.os == 'ios') ? ReactNativeBlobUtil.fs.dirs.CacheDir : ReactNativeBlobUtil.fs.dirs.DocumentDir;
  ...
  exists(path) {
    ...
    return ReactNativeBlobUtil.fs.exists(pathLib.resolve(this.baseFilePath + path));
  }
  ...
  async fetchFile(url, permanent = false, fileName = null, clobber = false) {
    ...
    try {
      ...
      result = await ReactNativeBlobUtil
        .config({
          path: path
        })
        .fetch('GET', url);
  ...
  async pruneCache() {
    ...
    // Get blobStatObjects.
    let lstat = await ReactNativeBlobUtil.fs.lstat(this.baseFilePath + 'cache');
    ...
    if (currentCacheSize > this.cachePruneTriggerLimit) {
      ...
      while (overflowSize > 0 && lstat.length) {
        ...
        if (!FileSystem.cacheLock[blobStatObject.filename] && this._validatePath('cache/' + blobStatObject.filename)) {
          ...
          ReactNativeBlobUtil.fs.unlink(this.baseFilePath + 'cache/' + blobStatObject.filename);
        }
  ...
  async unlink(path) {
    ...
    try {
      await ReactNativeBlobUtil.fs.unlink(pathLib.resolve(this.baseFilePath + path));
      return true;
    } catch (error) {
      return false;
    }
