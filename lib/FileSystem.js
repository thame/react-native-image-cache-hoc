...
export class FileSystem {
  ...
  _setBaseFilePath(fileDirName = null) {
    let baseFilePath = (this.os == 'ios') ? BlobCourier.fs.dirs.CacheDir : BlobCourier.fs.dirs.DocumentDir;
  ...
  exists(path) {
    ...
    return BlobCourier.fs.exists(pathLib.resolve(this.baseFilePath + path));
  }
  ...
  async fetchFile(url, permanent = false, fileName = null, clobber = false) {
    ...
    try {
      ...
      await BlobCourier.fs.unlink(path);
      throw error;
  ...
  async pruneCache() {
    ...
    // Get blobStatObjects.
    let lstat = await BlobCourier.fs.lstat(this.baseFilePath + 'cache');
    ...
    if (currentCacheSize > this.cachePruneTriggerLimit) {
      ...
      while (overflowSize > 0 && lstat.length) {
        ...
        if (!FileSystem.cacheLock[blobStatObject.filename] && this._validatePath('cache/' + blobStatObject.filename)) {
          ...
          BlobCourier.fs.unlink(this.baseFilePath + 'cache/' + blobStatObject.filename);
        }
  ...
  async unlink(path) {
    ...
    try {
      await BlobCourier.fs.unlink(pathLib.resolve(this.baseFilePath + path));
      return true;
    } catch (error) {
      return false;
    }
